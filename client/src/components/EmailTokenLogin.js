...
const EmailTokenLogin = () => {
  ...
  const sendOtp = async () => {
    setLoading(true);
    try {
      const res = await fetch('https://connnet4you-server.onrender.com/api/auth/send-otp', {
        method: 'POST',
  ...
  const verifyOtp = async () => {
    setLoading(true);
    try {
      const res = await fetch('https://connnet4you-server.onrender.com/api/auth/verify-otp', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },

...
async function createOrder({ items, total, address, paymentMethod, orderDate, userId }) {
  ...
  try {
    ...
    const orderRes = await client.query(
      ...
      [userId, total, address.name, address.street, address.city, address.zip, address.phone, paymentMethod, new Date(orderDate)]
    );
...
async function getOrdersByUser(userId) {
  ...
  try {
    const { rows: orders } = await client.query(
      `SELECT id, total, payment_method, order_date FROM orders
       WHERE user_id = $1 ORDER BY order_date DESC`, [userId]
    );
